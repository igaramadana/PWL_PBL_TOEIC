...
@section('content')
    <x-breadcrumb :pages="[
        ...
        ['name' => __('pendaftaran.editTitle'), 'url' => '#'],
    ]" />

    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('pendaftaran.editTitle') }}</h2>
    </div>
            ...
            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    <label for="nama_ujian" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('pendaftaran.EnterExamName') }}
                    </label>
                    <input type="text" name="nama_ujian" id="nama_ujian" value="{{ old('nama_ujian', $pendaftaran->nama_ujian) }}"
                ...
                <div>
                    <label for="jadwal_ujian" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('pendaftaran.ExamSchedule') }}
                    </label>
                    <input type="date" name="jadwal_ujian" id="jadwal_ujian" value="{{ old('jadwal_ujian', $pendaftaran->jadwal_ujian->format('Y-m-d')) }}"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-600 focus:border-primary-600 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        required>
                </div>
                <div>
                    <label for="waktu_ujian" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('pendaftaran.ExamTime') }}
                    </label>
                    <input type="time" name="waktu_ujian" id="waktu_ujian" value="{{ old('waktu_ujian', $pendaftaran->waktu_ujian_formatted ?? $pendaftaran->waktu_ujian_display) }}"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-600 focus:border-primary-600 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        required>
                </div>
                <div>
                    <label for="kuota" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        {{ __('pendaftaran.Quota') }}
                    </label>
                    <input type="number" name="kuota" id="kuota" min="1" value="{{ old('kuota', $pendaftaran->kuota) }}"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-600 focus:border-primary-600 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        required>
                </div>
            </div>
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div class="p-4 text-center md:p-5">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                        {{ __('pendaftaran.deleteConfirmation') }}</h3>
                    <div class="flex justify-between">
                        <form id="delete-form" action="{{ route('pendaftaran.destroy', $pendaftaran->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
