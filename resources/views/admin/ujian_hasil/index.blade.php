                ...
                <a href="{{ route('ujian_hasil.detail', $item->id) }}"
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-sm text-gray-600 dark:text-gray-300">
                                <span class="font-medium">Status Hasil Ujian:</span>
                                @if ($item->pendaftar_count > 0)
                                    <span
                                        class="px-2 py-1 text-xs rounded-full {{ $item->pendaftar->where('hasilUjian')->count() > 0 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                        {{ $item->pendaftar->where('hasilUjian')->count() > 0 ? 'Hasil tersedia' : 'Belum ada hasil' }}
                                    </span>
                                @else
